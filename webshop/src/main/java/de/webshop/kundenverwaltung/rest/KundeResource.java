...
public class KundeResource implements Serializable {
	...
	public Response findKundeById(@PathParam("id") Long id) {
		...
		if (kunde == null) {
			throw new NotFoundException(String.format(
					"Kein Kunde mit der ID {0] gefunden.", id));
		}
		setStructuralLinks(kunde, uriInfo);
		// Link-Header setzen
		final Response response = Response.ok(kunde)
				.links(getTransitionalLinks(kunde, uriInfo)).build();
		return response;
	}
	...
	public Response findKundeByNachname(
	...
	private Link[] getTransitionalLinksKunden(List<Kunde> kunden,
			UriInfo uriInfo2) {
	...
	public Response findBestellungenByKundeId(@PathParam("id") Long kundeId) {
		...
		if (kunde == null) {
			throw new NotFoundException(
					String.format(
							"Es wurden keine Bestellungen fuer den Kunden {0} gefunden",
							kundeId));
		}
	...
	private Link[] getTransitionalLinksBestellungen(
			List<Bestellung> bestellungen, Kunde kunde, UriInfo uriInfo2) {
